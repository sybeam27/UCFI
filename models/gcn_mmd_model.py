import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import GCNConv


class GCNMMD(nn.Module):
    def __init__(
        self,
        nfeat,
        hidden_dim=64,
        dropout=0.5,
        lambda_mmd=0.05,
        mmd_sample=256,
        lr=1e-3,
        weight_decay=1e-5,
        pos_weight=None,
    ):
        super().__init__()
        self.gcn1 = GCNConv(nfeat, hidden_dim)
        self.gcn2 = GCNConv(hidden_dim, hidden_dim)
        self.classifier = nn.Linear(hidden_dim, 1)
        self.dropout = dropout
        self.lambda_mmd = lambda_mmd
        self.mmd_sample_size = mmd_sample

        if pos_weight is not None and not torch.is_tensor(pos_weight):
            pos_weight = torch.tensor(float(pos_weight), dtype=torch.float32)

        self.pos_weight = pos_weight
        self.optimizer = torch.optim.Adam(
            self.parameters(), lr=lr, weight_decay=weight_decay
        )

        if pos_weight is None:
            self.criterion = nn.BCEWithLogitsLoss()
        else:
            self.criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)

    def compute_mmd(self, h, sensitive_attr):
        mask_0 = (sensitive_attr == 0)
        mask_1 = (sensitive_attr == 1)

        h_0 = h[mask_0]
        h_1 = h[mask_1]

        if h_0.size(0) > self.mmd_sample_size:
            idx_0 = torch.randperm(h_0.size(0), device=h.device)[:self.mmd_sample_size]
            h_0 = h_0[idx_0]

        if h_1.size(0) > self.mmd_sample_size:
            idx_1 = torch.randperm(h_1.size(0), device=h.device)[:self.mmd_sample_size]
            h_1 = h_1[idx_1]

        if h_0.numel() == 0 or h_1.numel() == 0:
            return torch.tensor(0.0, device=h.device)

        sigma = 1.0
        dist_xx = torch.cdist(h_0, h_0) ** 2
        dist_yy = torch.cdist(h_1, h_1) ** 2
        dist_xy = torch.cdist(h_0, h_1) ** 2

        xx = torch.exp(-dist_xx / (2 * sigma**2)).mean()
        yy = torch.exp(-dist_yy / (2 * sigma**2)).mean()
        xy = torch.exp(-dist_xy / (2 * sigma**2)).mean()

        return xx + yy - 2 * xy

    def forward(self, data):
        x = data.x
        edge_index = data.edge_index

        h = self.gcn1(x, edge_index)
        h = F.relu(h)
        h = F.dropout(h, self.dropout, training=self.training)

        h = self.gcn2(h, edge_index)
        h = F.relu(h)
        h = F.dropout(h, self.dropout, training=self.training)

        y_logit = self.classifier(h)
        return y_logit, h

    def optimize(self, data):
        labels = data.y.float()
        idx_train = data.idx_train
        idx_fair = data.idx_sens_train
        sensitive_attr = data.sensitive_attr

        self.train()
        self.optimizer.zero_grad()

        y_logit, h = self.forward(data)

        task_loss = self.criterion(
            y_logit[idx_train],
            labels[idx_train].unsqueeze(1)
        )

        if idx_fair.numel() > 0:
            h_fair = h[idx_fair]
            s_fair = sensitive_attr[idx_fair]
            mmd_loss = self.compute_mmd(h_fair, s_fair)
        else:
            mmd_loss = torch.tensor(0.0, device=y_logit.device)

        total_loss = task_loss + self.lambda_mmd * mmd_loss
        total_loss.backward()
        self.optimizer.step()

        return {
            "total_loss": total_loss.item(),
            "task_loss": task_loss.item(),
            "mmd_loss": mmd_loss.item(),
        }